"""
GUI/windows/declaration_window.py
...
class DeclarationWindow(QWidget):
    def __init__(
        self,
    ...
    def _validate_selected(self) -> None:
        ...
        if decl_id is None:
            return
        try:
            self._svc.validate_declaration(decl_id)
            self.load_data()
    ...
    def _reject_selected(self) -> None:
        ...
        if not ok or not reason.strip():
            return
        try:
            self._svc.reject_declaration(decl_id, reason.strip())
            self.load_data()
